...
Motivation: 解决混合模拟/数字接收器中由于模拟组合器导致的信号纠缠和低维投影问题，无法直接应用矩阵铅笔方法（MPM）的挑战。

Method: 提出两种方法：1）适用于全连接和部分连接HAD的周期性信号解缠方法；2）仅适用于部分连接HAD的块对角结构方法。
...
TL;DR: 研究了多设备MIMO通信中的波束跟踪问题，提出了非自适应二十问题估计方法，用于定位和跟踪多个移动目标，并推导了分辨率的非渐近和二阶渐近边界。


<details>
  <summary>Details</summary>
Motivation: 针对MIMO通信中多设备波束跟踪的实际应用需求，解决在查询依赖噪声信道下定位和跟踪多个移动目标的问题。

Method: 提出了一种非自适应二十问题估计方法，推导了最优查询程序的分辨率边界，并通过数值示例验证结果。采用单阈值解码规则降低计算复杂度。

Result: 证明了通过单阈值解码规则可以达到理论边界，计算复杂度低于多阈值方案。讨论了两种特殊情况（未知初始位置和已知速度，已知初始位置和未知速度），并推广到分段恒定速度模型。

Conclusion: 提出的算法适用于5G无线网络中多个移动发射机的波束跟踪，验证了方法的有效性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 森林环境中的通信系统对救援人员安全至关重要，但目前缺乏针对森林环境的信道检测与建模研究。

Method: 在内蒙古阿尔山国家森林公园进行了G2G和A2G信道测量，使用1.4 GHz的OFDM信号，分别采用全向和定向天线记录数据，并预规划无人机飞行轨迹。

Result: 提出的路径损耗模型误差较小，且发现A2G通信中树冠对信号传播的阻碍比G2G中树干更显著。调整空地对地仰角可提升通信质量。

Conclusion: 研究为森林环境中的通信系统设计提供了实用模型和关键参数分析，有助于优化通信性能。
...
TL;DR: 本文为LLM优化器（LLMO）框架首次建立了理论基础，通过将其解释为有限状态马尔可夫链并证明其收敛性，同时验证了多LLM架构对收敛速度的影响。


<details>
  <summary>Details</summary>
Motivation: 未来无线网络需要处理缺乏通用数学模型的多样化服务，LLM优化器框架被提出作为解决方案，但其理论支持尚未建立。

Method: 将LLMO框架解释为有限状态马尔可夫链，证明其收敛性，并扩展到多LLM架构，分析其对收敛速度的影响。

Result: 理论证明了LLMO框架的收敛性，并通过数值模拟验证了多LLM架构对收敛速度的提升。

Conclusion: 本文为LLMO框架提供了理论支持，并展示了其在多LLM架构下的潜力，为未来无线网络管理提供了新思路。
...
TL;DR: 该论文提出了一种利用可重构智能表面（RIS）辅助的集成感知与通信（ISAC）新框架，用于高精度结构健康监测（SHM）。


<details>
  <summary>Details</summary>
Motivation: 尽管ISAC在蜂窝系统中有广泛应用潜力，但其在需要检测缓慢细微结构变化的SHM中的应用尚未充分探索，主要受多径干扰和超高感知精度要求的挑战。

Method: 通过动态调整RIS相位生成独特无线电信号以抑制背景多径干扰，结合Fisher信息理论分析RIS辅助的三维蜂窝网络协作感知，并开发贝叶斯推理模型识别结构状态。

Result: 理论分析和数值模拟表明，ISAC可实现毫米级变形检测，满足SHM的高精度要求。

Conclusion: 该研究证明了RIS辅助的ISAC在高精度SHM中的潜力，为未来应用提供了理论支持。
...
<details>
  <summary>Details</summary>
Motivation: 可重构天线技术在6G及未来无线通信中具有潜力，但如何实时优化其系数以塑造理想信道仍是一个挑战，传统方法需要复杂的信道建模和参数估计。
...
TL;DR: 本文提出了一种改进的信号分离方法，用于处理交叉、低信噪比和不连续性的线性调频信号，并分析了噪声对信号分离的影响。


<details>
  <summary>Details</summary>
Motivation: 解决信号处理中叠加信号的分离问题，尤其是在交叉、低信噪比和不连续性情况下的挑战。

Method: 改进和扩展了Chui和Mhaskar的信号分离算子（SSO），用于分离线性调频信号，并分析了噪声对分离效果的影响。

Result: 通过理论分析和数值实验验证了方法的有效性，展示了在模拟数据集上的分离效果。

Conclusion: 改进的SSO方法能够有效处理复杂条件下的信号分离问题，为实际应用提供了理论支持。
...
TL;DR: 该研究提出了一种集成感知与通信（ISAC）的收发器，采用同时发射和反射的可重构智能表面（STAR-RIS）以及配备被动电子扫描阵列（PESA）和单数字通道的接收器。通过周期性脉冲信号和空间/时间调制，实现了双角度方向照明和通信消息嵌入，同时对雷达性能影响最小。


<details>
  <summary>Details</summary>
Motivation: 探索一种集成感知与通信的高效系统，通过STAR-RIS和PESA的结合，实现多目标探测和通信功能。

Method: 利用周期性脉冲信号在STAR-RIS上引入空间和时间调制，通过正交二进制码本实现通信消息嵌入，同时评估雷达性能。

Result: 提出的时间调制方法在保持雷达探测概率和径向速度估计精度的同时，实现了通信消息的嵌入。

Conclusion: 该方案为ISAC系统提供了一种高效且对雷达性能影响较小的通信嵌入方法。
...
TL;DR: 本文研究了考虑天线运动功耗和可调辐射功率的Pinching-antenna系统（PASS）设计，通过联合优化天线位置、辐射功率比和发射波束成形，最小化满足服务质量（QoS）要求的平均功耗。
...
Result: 仿真结果表明，考虑天线运动功耗和可调辐射功率能显著提升PASS性能，并验证了优化框架的高效性。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何利用配备感知和通信功能的基站，通过智能表面服务地面用户并扫描天空，以优化系统性能。

Method: 采用多帧雷达检测器（包括检测器、绘图提取器和检测前跟踪处理器），联合处理更多扫描以提升频谱效率。

Result: 数值分析验证了该方案的有效性，并评估了可实现的系统权衡。

Conclusion: 通过多帧雷达检测器联合处理扫描，可在保持感知性能的同时提升用户频谱效率。
...
TL;DR: 研究低空无线网络（LAWN）中无人机通过有限块长度（FBL）传输为多辆自动导引车（AGV）提供实时无线控制的优化问题，采用模型预测控制（MPC）和交替优化（AO）框架，验证了其控制性能的优越性。


<details>
  <summary>Details</summary>
Motivation: 低空无线网络（LAWN）在物联网（IoT）系统中具有潜力，但实时无线控制面临挑战，需优化控制策略、功率分配和无人机轨迹。

Method: 采用模型预测控制（MPC）确保轨迹跟踪，分析通信可靠性，并通过交替优化（AO）框架解决非凸优化问题，结合投影梯度下降（PGD）和连续凸近似（SCA）技术。

Result: 通过仿真和实验验证，提出的方法在控制性能上优于基线方案。

Conclusion: 该研究为LAWN系统中的实时无线控制提供了高效解决方案，优化了控制性能和通信可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在解决高速目标在通信-感知一体化系统中的参数估计问题，特别是针对存在距离迁移的情况。

Method: 利用OTFS调制信号处理，推导了高速目标回波的输入输出模型，提出基于块正交匹配追踪和匹配滤波器的最大似然估计器近似实现方法。

Result: 目标响应在延迟-多普勒域呈现稀疏结构，距离迁移导致响应扩散。数值实验验证了估计性能。

Conclusion: 研究为高速目标的参数估计提供了一种有效方法，特别是在存在距离迁移的场景中。
...
TL;DR: 论文探讨了注意力机制在资源分配中的必要性，通过分析集合上的函数结构和数值算法，证明注意力机制在特定情况下（如干扰未被反映时）是必要的，并提出了一个设计GNN的框架。


<details>
  <summary>Details</summary>
Motivation: 研究注意力机制在资源分配中的实际需求，以优化无线策略的学习效率和性能。

Method: 分析集合上的置换等变函数结构，重新表达数值算法的递归形式，设计GNN框架。

Result: 证明注意力机制在干扰未被反映时是必要的，并通过仿真验证了所提GNN的学习效率。

Conclusion: 注意力机制在特定场景下对资源分配至关重要，提出的GNN框架能有效提升学习效率。
...
TL;DR: 本文纠正了Rabiner等人和Lyons关于频率采样滤波器（FSF）设计中过渡系数和峰值旁瓣水平（PSL）的错误值，并提供了优化的数值。


<details>
  <summary>Details</summary>
Motivation: 发现Rabiner等人和Lyons在FSF设计中的过渡系数和PSL值存在显著差异且不准确，因此重新计算并提供最优值。

Method: 通过程序重新估计低通和带通滤波器的过渡系数及PSL值。

Result: 找到了最优的过渡系数和PSL值，例如对于N=16和BW=4，最优值为0.40474097。
...
<details>
  <summary>Details</summary>
Motivation: 在不增加硬件复杂度的前提下提高频谱效率，通过天线位置模式传递额外信息。

Method: 设计了全面的收发机方案，包括信道建模、低复杂度球解码算法（BOSD）、BER上界分析及基于流形优化的预编码方法。

Result: 仿真结果表明，PA-IM方案在性能上显著优于传统方法，且预编码设计大幅降低了系统BER。

Conclusion: PA-IM方案通过创新的收发机设计和优化方法，实现了频谱效率和性能的双重提升。
...
Method: 将混合波束成形问题分解为两个低维子问题，通过交替消除残差实现解耦，并采用几何信道SVD算法简化问题。

Result: AREE算法实现了低复杂度的解耦，快速收敛，且频谱效率优于现有方法。
...
TL;DR: 论文研究了毫米波MIMO系统中通过RIS增强传输的预编码设计，提出使用DNN代替传统穷举搜索以降低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决毫米波MIMO系统中直接通信路径受阻的问题，并优化预编码设计以提高吞吐量。

Method: 采用RIS增强传输，利用DNN快速选择码字，替代传统的穷举搜索和离散傅里叶变换方法。

Result: 仿真结果表明，DNN在测试阶段能保持次优的频谱效率，且对RIS与终端距离变化具有鲁棒性。
