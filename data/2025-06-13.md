...
<details>
  <summary>Details</summary>
Motivation: 随着信息存储和数据传输需求的增长，数据压缩技术变得愈发重要，尤其是在互联网和资源有限的移动设备时代。

Method: 通过综述多种数据压缩算法，分析其性能和应用。

Result: 数据压缩能有效节省存储和带宽，减少下载时间。

Conclusion: 数据压缩技术在当前和未来具有广泛应用和研究价值。
...
Method: 1. 分层训练方法分解复杂高码率码；2. 增强的神经架构（如dropout和可学习跳跃连接）；3. 渐进解冻策略。

Result: HiKO码在三阶和四阶码中超越Reed-Muller码，接近Shannon最优性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统DeepPolar码在BER表现优异但BLER性能不足，限制了其实际应用。本文旨在消除BER-BLER权衡。

Method: 提出三种创新：注意力增强解码器、结构化损失函数和自适应SNR匹配冗余技术（DP+SMART解码器）。
...
<details>
  <summary>Details</summary>
Motivation: 为论文中的主张提供理论支持，确保其有效性和可靠性。

Method: 通过技术报告的形式，详细阐述并证明论文中的各项主张。

Result: 提供了完整的证明，验证了论文中所有最优对齐的高效总结方法的正确性。

Conclusion: 技术报告成功支持了原论文的主张，为其提供了坚实的理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 研究群测试问题中近似恢复的可行性，特别是在仅允许单侧误分类（假阴性或假阳性）的情况下，探索算法的性能极限。

Method: 分析了两种单侧近似恢复标准：仅允许假阴性和仅允许假阳性。针对每种情况，分别提出了算法并进行了理论分析。

Result: 在仅允许假阴性的情况下，存在算法能达到与双侧近似恢复相同的优化阈值；在仅允许假阳性的情况下，证明了现有算法中的最优性能。

Conclusion: 论文表明，在单侧近似恢复条件下，算法的性能可以达到或接近最优，为群测试问题的实际应用提供了理论支持。
...
Method: 通过代数分析和构造性证明，研究加法循环码的互补对性质。

Result: 证明了加法互补对的构成码是自由模，并给出了互补对的充要条件，构造了一个互补对实例。

Conclusion: 有限交换环上的加法循环码互补对具有自由模性质，且存在明确的构造方法。
...
<details>
  <summary>Details</summary>
Motivation: 在频分双工系统中，上下行信道互易性不成立，导致下行信道状态信息获取困难。论文旨在通过优化反馈方案，提高信道估计的准确性。

Method: 利用经典的远程失真率理论，提出了理论下限，并设计了一种基于JSCC的实用反馈方案，利用信道二阶统计特性优化特征空间投影。

Result: 提出的JSCC方案在性能上优于已知的JSCC、SSCC基线及基于深度学习的方案，接近最优失真率方案在实际信噪比范围内的性能。

Conclusion: 论文提出的JSCC反馈方案在多用户大规模MIMO系统中显著提升了信道估计性能，为实际应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 物理层安全中的异常检测需要轻量计算、适应环境变化并抵抗相位噪声，现有方法需改进以满足这些需求。

Method: 采用移动平均、自回归和卡尔曼滤波预测特征向量，结合假设检验实现异常检测。

Result: 在WiFi设备案例中，算法与信道特征组合有效检测攻击；仅依赖发射功率随机化的防御措施不足。

Conclusion: 频率变化随机化可能是更有效的防御方案，需进一步研究。
...
TL;DR: 提出了一种基于深度学习的方法，利用语义分割模型（UNet和UNetPP）实现动态环境中的多源定位，效果优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 当前研究多集中于单源定位，动态环境中的多源定位研究存在空白，本文旨在填补这一空白。

Method: 使用UNet和UNetPP模型处理输入图像，结合平台位置和方向测量线，通过分析线的交点实现多源定位。

Result: 在单源定位中与传统方法相当，但在高噪声和多源条件下表现更优。

Conclusion: 深度学习框架在多源定位中具有潜力，尤其在复杂环境下表现突出。
...
<details>
  <summary>Details</summary>
Motivation: ICA在EEG处理中至关重要，但其性能依赖于数据质量和数量。研究旨在探索数据量与通道数对ICA分解质量的影响。

Method: 使用AMICA算法对71通道数据集进行分解，随机子采样以模拟不同数据量与通道数比例，通过MIR和近偶极性评估分解质量。

Result: 增加数据量导致MIR和近偶极性呈上升趋势，但未观察到明确的平台期，表明更多数据可能持续提升分解质量。

Conclusion: EEG数据收集的益处可能超出常见启发式阈值，持续增加数据量可能进一步提升ICA分解质量。
...
<details>
  <summary>Details</summary>
Motivation: 为可持续交通系统提供准确、经济、隐私保护且通用的主动出行监测方案。

Method: 利用DAS捕捉环境振动，结合深度学习推断运动模式；提出基于联邦学习的IoT方案和基于元学习的离线初始化方法以增强通用性。

Result: 实验验证了系统在步行和骑行分类中的性能和通用性。

Conclusion: 该系统为大规模主动出行监测提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统跑步机测量GRF成本高且不便携，而便携式鞋垫传感器易受噪声干扰且精度不足。
...
<details>
  <summary>Details</summary>
Motivation: 传统IRS因大量反射元素和控制电路导致功耗问题，HE-IRS通过结合DTEs和STEs来解决这一问题，但带来了信道估计和波束成形的新挑战。

Method: 将HE-IRS信道分解为DTE级联信道和STE等效信道，利用稀疏性和流形优化提出高效信道估计方案；针对波束成形，提出离线优化STE相移和在线优化BS预编码器及DTE相移的算法。

Result: 仿真结果表明，HE-IRS在相同元素数量下需要的导频开销更少，且通过提出的方案在显著降低功耗的同时保持了竞争性的总速率性能。

Conclusion: HE-IRS是一种绿色解决方案，通过结合DTEs和STEs，在降低功耗的同时保持了与传统IRS相当的性能。
...
<details>
  <summary>Details</summary>
Motivation: 通过RIS硬件架构和信道估计优化，提升MIMO通信系统的性能，尤其是在高频段（如FR3及以上）的应用。

Method: 采用单/多射频链接收导频信号，通过可调吸收相位剖面实现信道估计，并利用ADMM算法优化RIS反射系数。

Result: 数值实验表明，所提信道估计技术在多种系统和RIS硬件配置下优于基准方案，并能有效优化RIS单元的量化反射相位。

Conclusion: 该架构和信道估计协议显著提升了MIMO系统的性能，尤其适用于高频通信场景。
...
<details>
  <summary>Details</summary>
Motivation: PCI分配在5G网络中至关重要，但现有方法存在计算效率低和干扰问题，亟需高效解决方案。

Method: 将PCI分配问题分解为Min-3-Partition、Min-10-Partition和图着色问题，并采用惩罚镜像下降算法优化。

Result: 计算时间减少20倍，显著降低干扰并消除冲突，适用于大规模网络。

Conclusion: 该方法高效实用，能提升5G网络性能并降低部署成本。
...
<details>
  <summary>Details</summary>
Motivation: URLLC需要高可靠性和低复杂度的解码算法，现有方法在短块长度下性能不足或计算复杂度过高。

Method: 结合神经网络预测扰动模式，减少解码尝试次数，提升效率。

Result: 算法在保持准最大似然解码可靠性的同时，显著降低了计算复杂度。

Conclusion: 该方法适合时间敏感和资源受限的URLLC应用。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何利用夹持天线和多波导技术提升RSMA系统的总速率。

Method: 采用两步算法：1) 基于空间相关性和距离的低复杂度方法选择天线激活方案；2) 利用SDP-SCA优化波导波束成形。

Result: 仿真结果表明，所提方法在性能上优于多种基准方案。

Conclusion: 多波导夹持天线辅助RSMA系统能有效提升系统性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统FMCW LFI传感器在短距离、高速度、浅调制和低功耗条件下表现不佳，需要改进以适应可穿戴技术需求。

Method: 采用四斜坡调制方案解决拍频符号的模糊问题，并克服硬件限制导致的频谱盲区。

Result: 实现了稳健且低噪声的测量，性能优于现有硬件。

Conclusion: 该管道在紧凑、高效的可穿戴传感器应用中具有潜力。
...
<details>
  <summary>Details</summary>
Motivation: 汽车雷达的功能安全和可靠性对ADAS至关重要，通道不平衡是影响雷达可靠性的关键因素，需实时监测。

Method: 利用NLMS算法作为雷达处理链的一部分，输入为雷达的距离-多普勒图中的目标数据，无需目标的先验角度信息。

Result: 方法在多目标场景下表现优异，计算复杂度低，更新速率快，优于现有方法。

Conclusion: 该方法适用于在线实时监测雷达通道不平衡，提高了雷达系统的可靠性和安全性。
